...
x_min = None
x_max = None
# If turned on in terminal, splits the signal shape up into a pass signal and a fail signal
signal_pf = None
...
f"bin{i}": (f"pt_{lo}p00To{hi}p00", f"{lo:.2f}-{hi:.2f}")
for i, (lo, hi) in enumerate([
    (3,4), (4,5), (5,6), (6,7), (7,8), (8,9), (9,10), (10,30)
])
}  


def parse_arguments():
    """Parse all command line arguments and set global parameters."""
    global MASS, x_min, x_max, signal_pf, SIGNAL_MODELS, BACKGROUND_MODELS, FIT_CONFIGS
    
    parser = argparse.ArgumentParser(
        description="Fit ROOT histograms with different models."
    )
    parser.add_argument("--mass", required=True, choices=["Z", "JPsi", "JPsi_muon", "Z_muon"])
    parser.add_argument("--signal_pf", action="store_true")
    ...
    # Conditionally add --abseta for muon masses
    args, remaining = parser.parse_known_args()
    if args.mass == "Z_muon" or args.mass == "JPsi_muon":
        parser.add_argument("--abseta", required=True, type=int)
    
    #args = parser.parse_args()

    # Set global mass-dependent parameters
    MASS = args.mass
    signal_pf = args.signal_pf
    ...
    elif MASS in ["JPsi", "JPsi_muon"]:
        x_min, x_max = 2.7, 3.44  # GeV
        SIGNAL_MODELS = {
            "dcb": {
                "pdf": double_crystal_ball_pdf,
                "cdf": double_crystal_ball_cdf,
    ...
    # Define file paths and bin choices based on mass
    if MASS == "Z":
        file_paths = {
    ...
    parser.add_argument("--type", choices=FIT_CONFIGS.keys())
    parser.add_argument("--bin", choices=bins_choices)
...
def print_fit_summary(m, popt, perr, edges_pass, args_bin, BINS_INFO, Pearson_chi2, Poisson_chi2, total_ndof, args_data, fit_type, sigmoid_eff=False):
    ...
    if 'epsilon' in popt:
        if sigmoid_eff:
            ...
            print(f"  Efficiency ε     = {eff:.6f} ± {eff_err:.10f} (sigmoid)")
        else:
    ...
    if 'epsilon' in popt:
        try:
            ...
            if sigmoid_eff:
                ...
                eff_err = abs(perr.get('epsilon', float('nan')) * eff * (1 - eff))
                print("Efficiency:")
                print(f"  ε = {eff:.4f} ± {eff_err:.4f} (bin {lo:.1f} - {hi:.1f} GeV, sigmoid)")
            else:
                print("Efficiency:")
            ...
            if sigmoid_eff:
                eff = sigmoid(popt['epsilon'])
                eff_err = abs(perr.get('epsilon', float('nan')) * eff * (1 - eff))
                print(f"Efficiency: ε = {eff:.4f} ± {eff_err:.4f} (sigmoid)")
            else:
...
class PassFailPlotter:
    def __init__(self, cost_func_pass, error_func_pass, cost_func_fail, error_func_fail, n_bins_pass, edges_pass, edges_fail, fit_type, sigmoid_eff = False, signal_pf = False):
    ...
    def __call__(self, args):
        ...
        if self.signal_pf == True:
            signal_pass_params = [param_dict[f"{p}_pass"] for p in self.signal_param_names]
            signal_fail_params = [param_dict[f"{p}_fail"] for p in self.signal_param_names]
            signal_pass = self.signal_func(cx_pass, *signal_pass_params)
            signal_fail = self.signal_func(cx_fail, *signal_fail_params)
        else:
            signal_pass_params = [param_dict[p] for p in self.signal_param_names]
            signal_fail_params = [param_dict[p] for p in self.signal_param_names]
...
def create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=False, sigmoid_eff=False, signal_pf=False):
    ...
    if signal_pf == True:
        signal_pass_params = [params_dict[f"{p}_pass"] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
        signal_fail_params = [params_dict[f"{p}_fail"] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
    else:
        signal_pass_params = [params_dict[p] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
        signal_fail_params = [params_dict[p] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
...
def fit_function(fit_type, hist_pass, hist_fail, fixed_params=None, use_cdf=False, x_min=x_min, x_max=x_max, interactive=False, args_bin=None, args_data=None, sigmoid_eff=False, args_mass=None, signal_pf=False):
    ...
    def model_approx_pass(edges, *params):
        result_pass, _ = create_combined_model(fit_type, edges_pass, edges_fail, *params, sigmoid_eff=sigmoid_eff, signal_pf=signal_pf)
        return result_pass

    def model_approx_fail(edges, *params):
        _, result_fail = create_combined_model(fit_type, edges_pass, edges_fail, *params, sigmoid_eff=sigmoid_eff, signal_pf=signal_pf)
        return result_fail

    def model_cdf_pass(edges, *params):
        result_pass, _ = create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=True, sigmoid_eff=sigmoid_eff, signal_pf=signal_pf)
        return result_pass

    def model_cdf_fail(edges, *params):
        _, result_fail = create_combined_model(fit_type, edges_pass, edges_fail, *params, use_cdf=True, sigmoid_eff=sigmoid_eff, signal_pf=signal_pf)
        return result_fail
    ...
    # Interactive Fitter
    plotter = PassFailPlotter(c_pass, errors_pass, c_fail, errors_fail, len(edges_pass), edges_pass, edges_fail, fit_type, sigmoid_eff=sigmoid_eff, signal_pf=signal_pf)
    if interactive:
    ...
    if args_mass == "Z" or args_mass == "Z_muon":
        print_fit_summary(m, popt, perr, edges_pass, args_bin, BINS_INFO_Z, Pearson_chi2, Poisson_chi2, total_ndof, args_data, fit_type, sigmoid_eff=sigmoid_eff)
    elif args_mass == "JPsi_muon":
        print_fit_summary(m, popt, perr, edges_pass, args_bin, BINS_INFO_JPsi_muon, Pearson_chi2, Poisson_chi2, total_ndof, args_data, fit_type, sigmoid_eff=sigmoid_eff)
    elif args_mass == "JPsi": 
        print_fit_summary(m, popt, perr, edges_pass, args_bin, BINS_INFO_JPsi, Pearson_chi2, Poisson_chi2, total_ndof, args_data, fit_type, sigmoid_eff=sigmoid_eff)
...
def plot_combined_fit(results, plot_dir=".", data_type="DATA", fixed_params=None, sigmoid_eff=False, args_abseta=None, args_mass=None, signal_pf=False):
    if results is None:
    ...
    if signal_pf == True:
        signal_pass_params = [params[f"{p}_pass"] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
    else:
        signal_pass_params = [params[p] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
    bg_pass_params = [params[f"{p}_pass"] for p in BACKGROUND_MODELS[fit_type.split('_')[1]]["params"]]
    signal_pass =((params["N"] - (params["B_p"] + params["B_f"])) * eff * signal_func(x, *signal_pass_params))
    ...
    if args_mass == "Z" or args_mass == "Z_muon":
        ax_pass.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_Z[results['bin']][1]} GeV (Pass)", pad=10)
    elif args_mass == "JPsi_muon":
        ax_pass.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_JPsi_muon[results['bin']][1]} GeV (Pass)", pad=10)
    elif args_mass == "JPsi":
        ax_pass.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_JPsi[results['bin']][1]} GeV (Pass)", pad=10)
    ...
    if signal_pf == True:
        signal_fail_params = [params[f"{p}_fail"] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]
    else:
        signal_fail_params = [params[p] for p in SIGNAL_MODELS[fit_type.split('_')[0]]["params"]]   
    bg_fail_params = [params[f"{p}_fail"] for p in BACKGROUND_MODELS[fit_type.split('_')[1]]["params"]]
    signal_fail = ((params["N"] - (params["B_p"] + params["B_f"])) * (1-eff) * signal_func(x, *signal_fail_params))
    ...
    if args_mass == "Z" or args_mass == "Z_muon":
        ax_fail.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_Z[results['bin']][1]} GeV (Fail)", pad=10)
    elif args_mass == "JPsi_muon":
        ax_fail.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_JPsi_muon[results['bin']][1]} GeV (Fail)", pad=10)
    elif args_mass == "JPsi":
        ax_fail.set_title(f"{data_type.replace('_', ' ')}: {BINS_INFO_JPsi[results['bin']][1]} GeV (Fail)", pad=10)
...
def main():
    ...
    # Load data
    try:
        root_file = uproot.open(args.file_paths[args.data])
    except Exception as e:
        print(f"Error opening file: {e}")
        return

    # Prepare directories
    plot_dir = f"{args.mass}_fits/{args.data}/{args.bin}_fits/{'DATA' if args.data.startswith('DATA') else 'MC'}/{args.type}/"
    os.makedirs(plot_dir, exist_ok=True)

    print(f"DATA: {args.data}")
    
    # Load histograms based on mass type
    if args.mass == "Z":
        bin_suffix, bin_range = BINS_INFO_Z[args.bin]
        ...
        bin_suffix, bin_range = BINS_INFO_JPsi[args.bin]
        hist_pass = load_histogram(root_file, f"{args.bin}_{bin_suffix}_Pass", args.data)
        hist_fail = load_histogram(root_file, f"{args.bin}_{bin_suffix}_Fail", args.data)
    elif args.mass in ["JPsi_muon", "Z_muon"]:
        abseta = args.abseta
        pt = int(args.bin.split("bin")[1])
        bin_info = BINS_INFO_JPsi_muon if args.mass == "JPsi_muon" else BINS_INFO_Z
        bin_suffix, bin_range = bin_info[args.bin]
        
        hist_pass = load_histogram(root_file, 
            f"NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Pass", args.data)
        hist_fail = load_histogram(root_file, 
            f"NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Fail", args.data)
    ...
    results = fit_function(
        args.type, hist_pass, hist_fail, fixed_params,
        use_cdf=args.cdf, interactive=args.interactive,
        args_bin=args.bin, args_data=args.data, 
        sigmoid_eff=args.sigmoid, args_mass=args.mass, 
        signal_pf=args.signal_pf, x_min=args.x_min, x_max=args.x_max
    )
    ...
    # Store additional metadata in results
    results.update({
        "data_type": args.data,
        "type": args.type,
        "bin": args.bin
    ...
    plot_kwargs = {
        "plot_dir": plot_dir,
        "data_type": args.data,
        "sigmoid_eff": args.sigmoid,
        "args_mass": args.mass,
        "signal_pf": signal_pf
    }
    
    if args.mass in ["JPsi_muon", "Z_muon"]:
        plot_kwargs["args_abseta"] = args.abseta
    
    fig_pass, fig_fail = plot_combined_fit(results, **plot_kwargs)
