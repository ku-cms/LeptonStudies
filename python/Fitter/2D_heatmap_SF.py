...
df_data = df[df['Type'] == 'DATA'].copy()

df_data['bin_num'] = df_data['bin'].str.extract(r'bin(\d+)').astype(int)
...
for _, row in df_data.iterrows():
    ...
    if type == "DATA":
        heatmap_values[bin_num, col_idx] = row['SF']
        heatmap_errors[bin_num, col_idx] = row['SF_err']
    else:
        None

# Calculate color range
deviation = max(abs(np.nanmin(heatmap_values) - 1), abs(np.nanmax(heatmap_values) - 1))
vmin = 1 - deviation
vmax = 1 + deviation

# Create colormap
cmap = LinearSegmentedColormap.from_list("yellow_centered", ["red", "yellow", "red"])
...
plt.title("Scale Factors vs. $p_T$ and η Region", fontsize=16)
ax.invert_yaxis()

# Add values
for i in range(len(pt_labels)):  # rows = pT bins
    for j in range(len(eta_labels)):  # columns = eta regions
...
cbar.set_label("Scale Factor", fontsize=12)

# Save
plt.tight_layout()
plt.savefig("final_gold_blp_SF_heatmap.png")
plt.show()