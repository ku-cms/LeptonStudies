...
def main():
    parser_mass = argparse.ArgumentParser(add_help=False)
    parser_mass.add_argument("--mass", required=True, choices=["Z", "JPsi", "JPsi_muon", "Z_muon"])
    args_mass, remaining_argv = parser_mass.parse_known_args()
    ...
    if args_mass.mass == "Z":
        ...
        raise ValueError(f"Unknown mass: {args_mass.mass}")

    parser = argparse.ArgumentParser(
        description="Fit ROOT histograms with different models.",
        parents=[parser_mass]  # so we keep --mass already parsed
    )
    parser.add_argument("--bin", required=True, choices=bins_choices)
    ...
    if args_mass.mass == "JPsi_muon" or args_mass.mass == "Z_muon":
        parser.add_argument("--abseta", required=True, type=int, default=1)
    ...
    elif args.mass == "JPsi_muon":
        abseta = args.abseta
        pt = int(args.bin.split("bin")[1])
        # Load histograms
        bin_suffix, bin_range = BINS_INFO_JPsi_muon[args.bin]
        print(f"Bin suffix: {bin_suffix}")
        hist_pass = load_histogram(root_file, f"NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Pass", args.data)
        print(f"values: {hist_pass['values'] if hist_pass else 'None'}")
        print(f"edges: {hist_pass['edges'] if hist_pass else 'None'}")
        hist_fail = load_histogram(root_file, f"NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Fail", args.data)
        print(f"values: {hist_fail['values'] if hist_fail else 'None'}")
        print(f"edges: {hist_fail['edges'] if hist_fail else 'None'}")

        print(f"Looking for histograms:")
        print(f"Pass: NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Pass")
        print(f"Fail: NUM_gold_DEN_baselineplus_abseta_{abseta}_pt_{pt}_Fail")

    elif args.mass == "Z_muon":
        abseta = args.abseta
        pt = int(args.bin.split("bin")[1])
        # Load histograms
        bin_suffix, bin_range = BINS_INFO_Z[args.bin]
    ...
    results = fit_function(
        ...
        args_bin=args.bin, args_data=args.data, sigmoid_eff=args.sigmoid, args_mass=args.mass
    )
    if results is None:
    ...
    results["data_type"] = args.data
    results["type"] = args.type
    results["bin"] = args.bin
    
    
    # Plot results
    if args_mass.mass == "Z" or args_mass.mass == "JPsi":
        fig_pass, fig_fail = plot_combined_fit(results, plot_dir, args.data, sigmoid_eff=args.sigmoid, args_mass=args.mass)
    elif args_mass.mass == "JPsi_muon" or args_mass.mass == "Z_muon":
        fig_pass, fig_fail = plot_combined_fit(results, plot_dir, args.data, sigmoid_eff=args.sigmoid, args_abseta=args.abseta, args_mass=args.mass)
    plt.close(fig_pass)
    plt.close(fig_fail)
    root_file.close()
